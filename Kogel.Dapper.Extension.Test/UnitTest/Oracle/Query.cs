...
namespace Kogel.Dapper.Extension.Test.UnitTest.Oracle
{
   public class Query
    ...
    //                .ToList();
				...
				//		 Id = x.Id,
				//		 ArticleIds = x.ArticleId,
				//		 count = new QuerySet<News>(conn, new OracleSqlProvider()).Where(y => y.Id == x.ArticleId).Count(),
				//		 NewsList = new QuerySet<News>(conn, new OracleSqlProvider()).Where(y => y.Id == x.ArticleId).ToList(y => new NewsDto()
				//		 {
				//			 Id = y.Id,
				//			 Contents = y.Content
				//		 }).ToList(),
